...
export default function ReportsPage() {
  ...
  // Per-truck cost from cost tables
  ...
  const bestMonth   = [...monthData].sort((a,b)=>b.profit-a.profit)[0]
  const avgMargin   = totalRev > 0 ? (totalProfit/totalRev*100).toFixed(1) : '0.0'

  // ── Aging data — exclude Intake/no-date trucks ──
  ...
  // ── Cashflow data — cash in = sales, cash out = purchases + recon on their actual dates ──
  ...
  // Inflows: truck sales on date_sold
  soldTrucks.forEach(t => {
    const m = monthKey(t.date_sold!); if (!m) return
    ensureCash(m.key, m.label)
    cashByMonth[m.key].inflow += t.sold_price || 0
  })
  // Outflows: purchase price on bought_on date
  trucks.filter(t => t.bought_on).forEach(t => {
    const m = monthKey(t.bought_on!); if (!m) return
    ensureCash(m.key, m.label)
    cashByMonth[m.key].outflow += (t.purchase_price || 0) + (t.recondition_cost || 0)
  })
  // Outflows: vendor invoices on their own date (fall back to truck bought_on)
  vendorRows.forEach(r => {
    const truck = trucks.find(t => t.id === r.truck_id)
    const dateStr = truck?.bought_on; if (!dateStr) return
    const m = monthKey(dateStr); if (!m) return
    ensureCash(m.key, m.label)
    cashByMonth[m.key].outflow += r.amount
  })
  // Outflows: parts on truck bought_on date
  partRows.forEach(r => {
    const truck = trucks.find(t => t.id === r.truck_id)
    const dateStr = truck?.bought_on; if (!dateStr) return
    const m = monthKey(dateStr); if (!m) return
    ensureCash(m.key, m.label)
    cashByMonth[m.key].outflow += r.qty * r.unit_cost
  })
  // Outflows: labor on truck bought_on date
  laborRows.forEach(r => {
    const truck = trucks.find(t => t.id === r.truck_id)
    const dateStr = truck?.bought_on; if (!dateStr) return
    const m = monthKey(dateStr); if (!m) return
    ensureCash(m.key, m.label)
    cashByMonth[m.key].outflow += r.hours * r.rate
  })
  // Outflows: other costs on truck bought_on date
  otherRows.forEach(r => {
    const truck = trucks.find(t => t.id === r.truck_id)
    const dateStr = truck?.bought_on; if (!dateStr) return
    const m = monthKey(dateStr); if (!m) return
    ...
    const load = async () => {
      ...
      chartInstance.current = new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            x: { ticks: { color: label, font: { size: 9 }, maxRotation: 45, autoSkip: monthData.length > 18 }, grid: { color: grid }, border: { color: grid } },
            y: { ticks: { color: label, font: { size: 9 }, callback: (v: any) => fmtK(v) }, grid: { color: grid }, border: { color: grid } },
          },
        },
      })
    }
  ...
  const IS = { background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, padding: '8px 12px', color: 'var(--text)', fontSize: 12, outline: 'none', cursor: 'pointer' as const }
